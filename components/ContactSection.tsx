...
const ContactSection: React.FC<ContactSectionProps> = ({ 
  tenantId, 
  title = "Fale Connosco", 
  subtitle = "Estamos aqui para ajudar a esclarecer as suas dúvidas.",
  isWhiteLabel = false
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await LeadService.createLead(tenantId, {
        ...
        gdpr_consent_text: "Declaro que li e aceito os Termos de Uso, a Política de Privacidade e o Acordo de Tratamento de Dados (DPA) da plataforma Imosuite...",
  ...
  if (sent) {
    return (
      <section className="py-24 px-6 max-w-4xl mx-auto text-center animate-in zoom-in-95 duration-500">
  ...
  return (
    <section className="py-32 px-6 max-w-7xl mx-auto">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 text-slate-400 font-bold uppercase text-[10px] tracking-widest">
               <div className="h-px w-8 bg-slate-100"></div> Compromisso ImoSuite
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-[10px] text-slate-400 font-medium uppercase leading-relaxed">Encriptação de nível militar em todos os dados.</p>
               </div>
               <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                  <MessageSquare size={20} className="text-[#357fb2] mb-3" />
                  <p className="text-xs font-black text-[#1c2d51] uppercase mb-1">Resposta Rápida</p>
                  <p className="text-[10px] text-slate-400 font-medium uppercase leading-relaxed">Tempo médio de resposta inferior a 4 horas.</p>
               </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input required className="contact-input" placeholder="O seu nome" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Email</label>
                <input required type="email" className="contact-input" placeholder="email@exemplo.com" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Telemóvel (Opcional)</label>
              <input type="tel" className="contact-input" placeholder="+351 900 000 000" value={formData.telefone} onChange={e => setFormData({...formData, telefone: e.target.value})} />
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">A sua Mensagem</label>
              <textarea required rows={4} className="contact-input resize-none" placeholder="Em que podemos ajudar?" value={formData.mensagem} onChange={e => setFormData({...formData, mensagem: e.target.value})}></textarea>
            </div>
            ...
            <div className="space-y-4 py-6 border-y border-slate-50">
               <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <span className="text-[10px] font-medium text-slate-500 leading-normal group-hover:text-slate-700 transition-colors">
                    Declaro que li e aceito os <Link to="/termos" className="text-[#357fb2] underline">Termos de Uso</Link>, a <Link to="/privacidade" className="text-[#357fb2] underline">Política de Privacidade</Link> e o <Link to="/dpa" className="text-[#357fb2] underline">Acordo de Tratamento de Dados (DPA)</Link> da plataforma Imosuite, em conformidade com o Regulamento Geral sobre a Proteção de Dados (RGPD).
                  </span>
               </label>
               <label className="flex items-start gap-3 cursor-pointer group">
                  <input 
      ...
      <style>{`
        .contact-input { width: 100%; padding: 1.15rem 1.4rem; background: #f8fafc; border: 2px solid transparent; border-radius: 1.25rem; outline: none; font-weight: 700; color: #1c2d51; transition: all 0.2s; font-size: 0.875rem; }
        .contact-input:focus { background: #fff; border-color: #357fb2; }
      `}</style>
    </section>
  );
};
