...
const ContactSection: React.FC<ContactSectionProps> = ({ 
  ...
  if (sent) {
    return (
      <section className="py-24 px-6 max-w-4xl mx-auto text-center animate-in zoom-in-95 duration-500 bg-white rounded-[3rem] my-10 shadow-sm border border-slate-100 text-slate-900">
        ...
        <h2 className="text-3xl font-black mb-4 tracking-tighter text-[#1c2d51]">Mensagem Recebida!</h2>
    ...
    <section className="py-32 px-6 max-w-7xl mx-auto font-brand">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
        <div className="space-y-8">
          <div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tighter leading-tight mb-6 text-white">{title}</h2>
            <p className="text-xl font-medium leading-relaxed max-w-md text-white/80">{subtitle}</p>
          </div>
          
          <div className="space-y-6">
            <div className="flex items-center gap-4 text-white/40 font-bold uppercase text-[10px] tracking-widest">
               <div className="h-px w-8 bg-white/20"></div> Compromisso de Qualidade
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <div className="p-6 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 shadow-sm">
                  <ShieldCheck size={20} className="text-white mb-3" />
                  <p className="text-xs font-black text-white uppercase mb-1">Dados Seguros</p>
                  <p className="text-[10px] text-white/60 font-medium uppercase leading-relaxed">Privacidade protegida sob as normas do RGPD.</p>
               </div>
               <div className="p-6 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 shadow-sm">
                  <MessageSquare size={20} className="text-white mb-3" />
                  <p className="text-xs font-black text-white uppercase mb-1">Apoio Dedicado</p>
                  <p className="text-[10px] text-white/60 font-medium uppercase leading-relaxed">Resposta personalizada para cada pedido.</p>
               </div>
            </div>
          </div>
        </div>

        <div className="bg-white p-10 md:p-14 rounded-[3rem] shadow-2xl text-slate-900 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 py-6 border-y border-slate-50">
               <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <span className="text-[10px] font-medium text-slate-500 leading-normal group-hover:text-slate-700 transition-colors">
                    Declaro que li e aceito os <Link to="/termos" className="text-[#357fb2] underline">Termos de Uso</Link> e a <Link to="/privacidade" className="text-[#357fb2] underline">Política de Privacidade</Link>.
                  </span>
               </label>
            </div>
            ...
            <button 
              type="submit" 
              disabled={isSending} 
              className="w-full bg-[#1c2d51] text-white py-6 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl flex items-center justify-center gap-3 hover:-translate-y-1 transition-all disabled:opacity-50"
            >
