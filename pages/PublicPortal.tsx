...
const PublicPortal: React.FC = () => {
  ...
  const getMenuLink = (path: string) => {
    ...
    // Se não for rota especial, assume-se que é uma página (/p/slug)
    return `/agencia/${tenant.slug}/p/${path.replace(/^\//, '').replace(/^p\//, '')}`;
  };
