...
const PublicPortal: React.FC = () => {
  ...
  const styles: Record<string, any> = {
    heritage: { nav: "h-20 md:h-28 px-6 md:px-8 sticky top-0 z-50 bg-white border-b border-slate-100 flex items-center justify-between", navText: "font-heritage italic text-[#1c2d51]", button: "bg-[var(--primary)] text-white px-8 py-4 font-black uppercase text-xs", heading: "font-heritage italic text-[#1c2d51]", footer: "py-20 md:py-24 px-6 md:px-10 bg-[var(--primary)] text-white" },
    canvas: { nav: "h-20 md:h-32 px-6 md:px-12 sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-50 flex items-center justify-between", navText: "font-black tracking-tight text-[#1c2d51]", button: "bg-[var(--primary)] text-white px-8 py-4 rounded-2xl font-black uppercase text-xs shadow-lg", heading: "font-black text-[#1c2d51] tracking-tight", footer: "py-20 md:py-24 px-6 md:px-12 bg-[var(--primary)] text-white" },
    prestige: { nav: "h-20 md:h-28 px-6 md:px-10 sticky top-0 z-50 bg-black text-white border-b border-white/5 flex items-center justify-between", navText: "font-black italic", button: "bg-white text-black px-10 py-4 font-black uppercase text-[10px]", heading: "font-black italic uppercase text-white", footer: "py-20 md:py-24 px-6 md:px-10 bg-[var(--primary)] text-white border-t border-white/5" },
    skyline: { nav: "h-20 md:h-28 px-6 md:px-8 sticky top-0 z-50 bg-[var(--primary)] text-white flex items-center justify-between", navText: "font-black uppercase", button: "bg-white text-[var(--primary)] px-8 py-3 rounded-xl font-black uppercase text-xs shadow-xl", heading: "font-black uppercase text-[#1c2d51]", footer: "py-20 md:py-24 px-6 md:px-10 bg-[var(--primary)] text-white" },
    luxe: { nav: "h-24 md:h-32 px-6 md:px-12 sticky top-0 z-50 bg-[#FDFBF7]/90 backdrop-blur-sm flex items-center justify-between", navText: "font-black text-[#2D2926]", button: "bg-[#2D2926] text-white px-10 py-5 rounded-[2.5rem] font-bold text-xs uppercase shadow-2xl", heading: "font-black text-[#2D2926] tracking-widest", footer: "py-20 md:py-24 px-6 md:px-12 bg-[var(--primary)] text-white" }
  };
      ...
      <main className="flex-1 animate-in fade-in duration-700 overflow-x-hidden">
        {cms.homepage_sections.filter(sec => sec.enabled).sort((a,b) => a.order - b.order).map((section) => {
          if (section.type === 'hero') return (
            <section key={section.id} className="relative h-[70vh] md:h-[85vh] flex items-center overflow-hidden">
               ...
               <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-10 w-full text-white text-center">
                  ...
                  <p className="text-base md:text-xl opacity-80 mb-8 md:mb-10 max-w-2xl mx-auto font-medium">{section.content.subtitle}</p>
                  <Link to={`/agencia/${tenant.slug}/imoveis`} className="bg-white text-[#1c2d51] px-8 py-4 md:px-12 md:py-6 rounded-2xl font-black text-xs md:text-base uppercase shadow-2xl inline-block">Ver Propriedades</Link>
               </div>
            </section>
          );
          if (section.type === 'featured') return (
            <section key={section.id} className="py-16 md:py-24 max-w-7xl mx-auto px-6">
               <h2 className={`text-3xl md:text-4xl mb-8 md:mb-12 ${s.heading}`}>{section.content.title}</h2>
          ...
          if (section.type === 'about_mini') return (
            <section key={section.id} className="py-16 md:py-24 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-20 items-center">
               <div className={`aspect-square overflow-hidden shadow-2xl ${tid === 'luxe' ? 'rounded-[3rem] md:rounded-[4rem]' : tid === 'canvas' ? 'rounded-[2rem] md:rounded-[3rem]' : ''}`}>
               ...
               <div className="space-y-4 md:space-y-6">
                  ...
                  <Link to={`/agencia/${tenant.slug}/p/quem-somos`} className="inline-flex items-center gap-2 text-[10px] md:text-xs font-black uppercase tracking-widest text-[var(--primary)] border-b-2 border-current pb-1">Saber Mais</Link>
               </div>
            </section>
          );
          return null;
      ...
      <footer className={s.footer}>
         <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-20">
            <div className="space-y-4 md:space-y-6 text-center md:text-left">
               <h4 className="text-lg md:text-xl font-black uppercase">{tenant.nome}</h4>
               <p className="text-xs md:text-sm opacity-70">{tenant.slogan}</p>
            </div>
