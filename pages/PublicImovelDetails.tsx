...
const PublicImovelDetails: React.FC = () => {
      ...
      try {
        ...
        if (!tSnap.empty) {
          ...
          if (!iSnap.empty) {
            ...
            // BUSCA OBRIGATÓRIA DA GALERIA NA SUBCOLEÇÃO
  ...
  const handleContact = async (e: React.FormEvent) => {
    ...
    try {
      await LeadService.createLead(tenant.id, { 
        ...
        mensagem: formData.mensagem || `Olá, gostaria de saber mais sobre o imóvel ${imovel.ref}.`
      });
      setSent(true);
    } catch (err) {
  ...
  // Prioriza a média carregada da subcoleção, fallback para cover_url
    ...
    <div className="min-h-screen flex flex-col bg-white font-brand">
      ...
      <nav className="h-20 px-8 flex items-center justify-between sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-slate-50">
         <Link to={`/agencia/${tenant.slug}`} className="flex items-center gap-3">
            {tenant.logo_url ? <img src={tenant.logo_url} className="h-10 w-auto object-contain" alt={tenant.nome} /> : <span className="font-black text-xl uppercase text-[var(--primary)]">{tenant.nome}</span>}
         </Link>
         <div className="hidden md:flex gap-10">
            {cms.menus.main.map(m => (
              <Link key={m.id} to={m.path.startsWith('http') ? m.path : `/agencia/${tenant.slug}${m.path === '/' ? '' : '/p/' + m.path.replace('/', '')}`} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-[var(--primary)]">{m.label}</Link>
            ))}
         </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full animate-in fade-in duration-700">
        <Link to={`/agencia/${tenant.slug}`} className="inline-flex items-center gap-2 text-slate-400 text-[10px] font-black uppercase mb-10 hover:text-[var(--primary)]">
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="relative aspect-[16/9] rounded-[3.5rem] overflow-hidden bg-slate-100 shadow-2xl group">
               ...
               <div className="absolute top-8 left-8">
                  <span className="bg-white/90 backdrop-blur px-5 py-2 rounded-xl text-[10px] font-black uppercase text-[#1c2d51] shadow-lg">
                    {imovel.operacao}
                  </span>
               </div>
            ...
            <div className="space-y-10 pt-6">
               ...
               <div className="bg-slate-50/50 p-10 md:p-14 rounded-[4rem] border border-slate-100">
                  <h3 className="text-2xl font-black text-[#1c2d51] uppercase mb-8 flex items-center gap-3">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="sticky top-28 space-y-6">
              <div className="bg-[#1c2d51] p-12 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden group">
                 <p className="text-[10px] font-black uppercase text-blue-300 mb-4">Valor</p>
              ...
              <div className="bg-white p-10 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-xl space-y-8">
                 <h3 className="font-black text-[#1c2d51] text-2xl uppercase">Contactar</h3>
                 
                 {sent ? (
                   <div className="py-12 text-center space-y-4 animate-in zoom-in-90">
                      <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-3xl flex items-center justify-center mx-auto shadow-inner"><Check size={40} /></div>
                      <h3 className="text-xl font-black text-[#1c2d51]">Enviado!</h3>
                   ...
                   <form onSubmit={handleContact} className="space-y-4">
                      ...
                      <textarea required rows={5} placeholder="Mensagem" className="detail-input-v2 resize-none" value={formData.mensagem} onChange={e => setFormData({...formData, mensagem: e.target.value})} />
                      <label className="flex items-start gap-4 cursor-pointer pt-4">
                         <input type="checkbox" required checked={gdprConsent} onChange={e => setGdprConsent(e.target.checked)} className="mt-1 w-5 h-5 rounded-lg border-slate-200 text-[#1c2d51]" />
                         <span className="text-[10px] font-medium text-slate-400">Aceito o tratamento dos meus dados comerciais.</span>
                      </label>
                      <button type="submit" disabled={isSending} className="w-full bg-[#1c2d51] text-white py-6 rounded-[2rem] font-black text-sm uppercase shadow-2xl flex items-center justify-center gap-3 hover:-translate-y-1 disabled:opacity-50 transition-all">
