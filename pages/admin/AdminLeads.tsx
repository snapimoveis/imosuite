...
import { Mail, Phone, Calendar, MessageSquare, CheckCircle2, Clock, Loader2, User, ExternalLink } from 'lucide-react';
            ...
            <div key={lead.id} className={`bg-white p-8 rounded-[2.5rem] border transition-all hover:shadow-lg flex flex-col md:flex-row justify-between gap-6 ${!lead.lido ? 'border-l-4 border-l-blue-500 border-slate-100' : 'border-slate-100 opacity-80'}`}>
              <div className="flex gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-600 bg-slate-50 p-4 rounded-2xl italic">"{lead.mensagem}"</p>
                </div>
              </div>
              <div className="flex flex-col justify-between items-end gap-4">
                ...
                <button 
                  ...
                  disabled={lead.lido}
                  className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${!lead.lido ? 'bg-[#1c2d51] text-white shadow-lg' : 'bg-slate-100 text-slate-400 cursor-default'}`}
                >
                  {lead.lido ? 'Respondido' : 'Marcar como Lido'}
                </button>
              </div>
            </div>
          ))
