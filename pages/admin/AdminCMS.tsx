...
const AdminCMS: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const finalCMS = { ...cms, homepage_sections: processedSections, pages: processedPages };
                  ...
                  <div key={page.id} className={`bg-white p-6 rounded-[2rem] border transition-all flex flex-col justify-between group ${page.enabled ? 'border-slate-100 shadow-sm' : 'border-dashed border-slate-200 opacity-60'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {!cms.menus.main.some(m => m.path === page.slug || m.path === `/p/${page.slug}`) && (
                           <button 
                             onClick={() => {
                               setMenuTarget('main');
                               setMenuLinkType('page');
                ...
                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      ...
                      <p className="text-[8px] text-slate-400 font-bold uppercase">Links visíveis no topo do site</p>
                    </div>
                  ...
                  <div className="space-y-2">
                    {cms.menus.main.map((item) => (
                      <div key={item.id} className="p-4 bg-slate-50 rounded-2xl flex items-center justify-between group">
                         ...
                         <button onClick={() => setCms({...cms, menus: {...cms.menus, main: cms.menus.main.filter(m => m.id !== item.id)}})} className="p-2 text-slate-200 group-hover:text-red-500 transition-colors"><Trash2 size={16}/></button>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      ...
                      <p className="text-[8px] text-slate-400 font-bold uppercase">Links legais ou institucionais</p>
                    </div>
                  ...
                  <div className="space-y-2">
                    {cms.menus.footer.map(item => (
                      <div key={item.id} className="p-4 bg-slate-50 rounded-2xl flex items-center justify-between group">
                         ...
                         <button onClick={() => setCms({...cms, menus: {...cms.menus, footer: cms.menus.footer.filter(m => m.id !== item.id)}})} className="p-2 text-slate-200 group-hover:text-red-500 transition-colors"><Trash2 size={16}/></button>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b flex items-center justify-between">
              <h3 className="text-xl font-black text-[#1c2d51] tracking-tight">Vincular Link de Menu</h3>
            ...
            <div className="p-8 space-y-6">
              ...
              {menuLinkType === 'page' ? (
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {cms.pages.map(p => <option key={p.id} value={p.slug}>{p.title} (/p/{p.slug})</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Texto no Menu</label>
                    <input className="admin-input-cms" value={editingMenuItem.label} onChange={e => setEditingMenuItem({...editingMenuItem, label: e.target.value})} placeholder="Ex: Portal Parceiros" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-2">URL ou Caminho</label>
              ...
              <button 
                onClick={() => {
                  if (!editingMenuItem.label || !editingMenuItem.path) return alert("Preencha todos os campos.");
                  setCms(prev => ({
                    ...prev,
                ...
                className="w-full bg-[#1c2d51] text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl flex items-center justify-center gap-3"
              >
                <Link2 size={16}/> Adicionar ao Menu
              </button>
            </div>
          </div>
        </div>
